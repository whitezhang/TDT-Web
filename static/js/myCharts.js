...
var chart = new Chart(document.getElementById("trendsLineChart").getContext("2d")).Line(lineChartData, options);

function showMyTrend(dataName) {
    dataId = -1;
    for (var i = 0; i < nameSet.length; i++) {
        if (dataName == nameSet[i]) {
            dataId = i;
            break;
        }
    }
    ...
    lineChartData["datasets"][dataId]["fillColor"] = "rgba(88,220,220,1)";
    lineChartData["datasets"][dataId]["strokeColor"] = "rgba(88,220,220,1)";
    lineChartData["datasets"][dataId]["pointColor"] = "rgba(88,220,220,1)";

    chart = new Chart(document.getElementById("trendsLineChart").getContext("2d")).Line(lineChartData, options);
};